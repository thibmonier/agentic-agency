...
describe("ApplicationsMetierPage", () => {
  it("renders the service template with correct data", () => {
    render(<Page />);
    expect(screen.getByTestId("service-template")).toBeInTheDocument();
    expect(screen.getByText("Applications métier")).toBeInTheDocument();
  });

  it("has correct CTA link", () => {
    render(<Page />);
    expect(screen.getByText("CTA")).toHaveAttribute("href", "/contact?sujet=applications-metier");
  });
});
